    ...
    for key in episodes_list:
        ...
        for line in text:
            ...
            if "Normal quality" in line:
                line = line.split("(")[1].split(")")[0].replace("'", "").replace(",", " ").split(" ")
                link_normal = f"https://sbplay.one/dl?op=download_orig&id={line[0]}&mode=n&hash={line[2]}"
                episodes[key]["sbplay_normal_page"] = link_normal
            if "Low quality" in line:
                line = line.split("(")[1].split(")")[0].replace("'", "").replace(",", " ").split(" ")
                link_low = f"https://sbplay.one/dl?op=download_orig&id={line[0]}&mode=l&hash={line[2]}"
